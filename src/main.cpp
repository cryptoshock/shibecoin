...
const string strMessageMagic = "ShibeCoin Signed Message:\n";
...
// miner's coin base reward based on height in blockchain
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight <= 1550)
    {
        nSubsidy = 75000 * COIN;
    }
    else if(nHeight <= 3550)
    {
	nSubsidy = 50000 * COIN;
    }
    else if(nHeight <=  6300)
    {
	nSubsidy = 25000 * COIN;
    }
    else
    {
	// No block reward subsidy after 300M coins from PoW
	nSubsidy = 0 * COIN;
    }
...
{
    static int64 nRewardCoinYear = 0.10 * COIN;  // 10% interest, creation amount per coin-year
...
static const int64 nTargetTimespanDS = 60; // 60 seconds
static const int64 nTargetSpacingDS = 60; // 60 seconds
static const int64 nIntervalDS = nTargetTimespanDS / nTargetSpacingDS; // 1 block
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
	...
	int64 retargetTimespan = nTargetTimespanDS;
	int64 retargetSpacing = nTargetSpacingDS;
    	int64 retargetInterval = nIntervalDS;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ShibeCoin
    bool fStrictPayToScriptHash = true; // Always active in ShibeCoin
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRI64x" nStakeModifierChecksum: 0x%016"PRI64x, pindexNew->nHeight, nStakeModifier, pindexNew->nStakeModifierChecksum);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ShibeCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 01-MAY-2014 Nasa shows off prototype spacesuit for journey to Mars";
        CTransaction txNew;
        txNew.nTime = 1399004008;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	uint256 hash = block.GetHash();
        ...
        assert(block.hashMerkleRoot == uint256("0x522d64e3e54d1ee830d4e5e02dd3d31ea9a3ad75a50dbd82ee06878535448bfe"));
...
unsigned char pchMessageStart[4] = { 0xf2, 0xf6, 0xe1, 0xc0 };
